...
app/
  ...
  server.py          FastAPI REST API endpoints and WebSocket log streaming
...
The active UI is the HTML frontend in `app/static/` served by `app/server.py`. Legacy PyQt6/QML code has been removed.
...
- **API routes** in `app/server.py` are grouped by domain (`/api/profiles`, `/api/proxies`, etc.) and use the `api_` prefix for handler function names.
...
- Heavy runtime dependencies such as `BrowserInterface` are imported lazily inside API handlers to keep server startup lightweight.
...
- Pull requests should describe what changed and why. Include screenshots or screen recordings for UI-altering SPA changes.
